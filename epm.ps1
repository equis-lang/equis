...
switch ($Command) {
    "init" {
        if (Test-Path "equis.json") { Write-Error "[epm] Error: equis.json already exists in this directory."; return }
        $name = Split-Path -Leaf (Get-Location)
        $json = @{ name = $name; version = "0.1.0"; dependencies = @{} } | ConvertTo-Json
        ...
        if (-not (Test-Path ".gitignore")) { "*.ll`n*.o`n*.exe`n.equis/" | Out-File -FilePath ".gitignore" -Encoding utf8 }
        Write-Host "[epm] Initialized Equis project: $name" -ForegroundColor Green
    }
    "get" {
        if ($Args.Count -eq 0) { Write-Host "Usage: epm get <repo_url>" -ForegroundColor Yellow; return }
        $repo = $Args[0]
    ...
    "install" {
        if (-not (Test-Path "equis.json")) { Write-Error "[epm] No equis.json found."; return }
        $config = Get-Content "equis.json" | ConvertFrom-Json
    ...
    "run" {
        ...
        if (Test-Path $eqPath) { & powershell -File $eqPath run $Args }
        else { & eq run $Args }
    }
    default {
