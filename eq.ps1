...
switch ($Command) {
    "compile" {
        ...
        if (-not $InFile) {
            Write-Host "Usage: eq compile <file.equis> [-o <output>]" -ForegroundColor Yellow
            return
        }
    ...
    "check" {
        if (-not $Args) {
            Write-Host "Usage: eq check <file.equis>" -ForegroundColor Yellow
            return
        }
    ...
    "lsp" {
        & $EQUIS_CORE $INCLUDE_ARGS[0] $INCLUDE_ARGS[1] --lsp
    }
    
    "build" {
        Write-Host "[eq] Rebuilding self-hosted toolchain..." -ForegroundColor Cyan
        ...
        Write-Host "[eq] Local toolchain synchronized." -ForegroundColor Green
    }
    ...
    "version" {
        Write-Host "Equis Professional Toolchain v0.1.0 (Windows-Native)" -ForegroundColor Cyan
    }
